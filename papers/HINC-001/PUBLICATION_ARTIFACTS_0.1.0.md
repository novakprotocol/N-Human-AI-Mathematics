# HINC-001 candidate publication artifacts — version 0.1.0

## Source identity

```text
repository:             novakprotocol/N-Human-AI-Mathematics
branch:                 release/public-review-v1
artifact source freeze: 16202759b857bc1f7f92e514531936a67737edf8
release label:          0.1.0-public-review-candidate
```

The artifact source freeze identifies the controlling manuscript and review-guide content used to build the derived files. Later workflow and release-control commits may advance the branch without changing the governed manuscript, guide, or binary artifact bytes.

The derived artifacts are prepared for attachment to the eventual GitHub public-review release. They are not committed as binary files in the source history.

## Complete package

```text
file:
HINC-001_Candidate_Technical_Review_Package_0.1.0.zip

SHA-256:
b357909077792a3e3b124359fa7348c21aea72c5d802c19562b46fa96350c123
```

## Included reading artifacts

- `HINC-001_Revised_Manuscript_0.1.0.md`
- `HINC-001_Revised_Manuscript_0.1.0.docx`
- `HINC-001_Revised_Manuscript_0.1.0.pdf`
- `HINC-001_Public_Review_Guide_0.1.0.md`
- `HINC-001_Public_Review_Guide_0.1.0.docx`
- `HINC-001_Public_Review_Guide_0.1.0.pdf`
- DOCX accessibility receipts
- PDF preflight receipts
- artifact QA receipt
- SHA-256 manifest

## QA

```text
revised manuscript pages:         15
review guide pages:                4
DOCX accessibility findings:      0
PDF preflight warnings:            0
PDF encryption:                    no
image-only PDFs:                   no
visual page inspection:            complete
```

## Release boundary

The artifact package is a **candidate technical-review package**. It does not establish:

- external specialist validation;
- independent external reproduction;
- peer review;
- journal acceptance;
- worldwide historical priority;
- complete Lean formalization;
- a specific model or provider identity.

The final public-switch process must verify the ZIP hash above before attaching it to a GitHub release.
